"""Contract-first Wintermute career source backed by Lever postings JSON."""
...
_BOARD_URL = "https://api.lever.co/v0/postings/wintermute-trading?mode=json"
_SOURCE_ID = "career:wintermute"
_COMPANY = "Wintermute"
_REQUIREMENTS_LABEL_MARKERS = ("requirements", "required", "hard skills", "highly preferred")


class WintermuteCareerSource(SourceScraper):
    @property
...
def _json_array(body: str) -> list[Any]:
    ...
    if not isinstance(value, list):
        raise ValueError("Wintermute Lever response is not a JSON array")
    return value
...
def _categories(value: object) -> dict[str, object]:
    if not isinstance(value, dict):
        raise ValueError("Wintermute Lever posting categories are malformed")
    return dict(value)
...
def _required_text(value: object, field_name: str) -> str:
    text = _text(value).strip()
    if not text:
        raise ValueError(f"Wintermute Lever posting is missing {field_name}")
    return text
