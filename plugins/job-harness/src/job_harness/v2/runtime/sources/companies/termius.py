"""Contract-first Termius career source backed by Lever postings JSON."""
...
_BOARD_URL = "https://api.lever.co/v0/postings/Termius?mode=json"
_SOURCE_ID = "career:termius"
_COMPANY = "Termius"
_REQUIREMENTS_LABEL_MARKERS = ("qualifications", "requirements", "required", "looking for")


class TermiusCareerSource(SourceScraper):
    @property
...
def _json_array(body: str) -> list[Any]:
    ...
    if not isinstance(value, list):
        raise ValueError("Termius Lever response is not a JSON array")
    return value
...
def _categories(value: object) -> dict[str, object]:
    if not isinstance(value, dict):
        raise ValueError("Termius Lever posting categories are malformed")
    return dict(value)
...
def _required_text(value: object, field_name: str) -> str:
    text = _text(value).strip()
    if not text:
        raise ValueError(f"Termius Lever posting is missing {field_name}")
    return text
