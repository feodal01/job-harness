"""Contract-first airSlate career source backed by Lever postings JSON."""
...
_BOARD_URL = "https://api.lever.co/v0/postings/airslate?mode=json"
_SOURCE_ID = "career:airslate"
_COMPANY = "airSlate"
_REQUIREMENTS_LABEL_MARKERS = ("need", "requirements", "expect", "looking for")


class AirSlateCareerSource(SourceScraper):
    @property
...
def _json_array(body: str) -> list[Any]:
    ...
    if not isinstance(value, list):
        raise ValueError("airSlate Lever response is not a JSON array")
    return value
...
def _categories(value: object) -> dict[str, object]:
    if not isinstance(value, dict):
        raise ValueError("airSlate Lever posting categories are malformed")
    return dict(value)
...
def _required_text(value: object, field_name: str) -> str:
    text = _text(value).strip()
    if not text:
        raise ValueError(f"airSlate Lever posting is missing {field_name}")
    return text
