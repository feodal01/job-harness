...
async def resolve_site(
    ...
    *,
    ...
    if career_url is None:
        return SiteResolution(channel=None, attempted=True, resolved=False, failed=False)
    return SiteResolution(
        channel=_career_channel(
            career_url,
            status="resolved",
...
def _career_link_score(
    *,
    ...
    parsed = urlparse(url)
    if _is_non_employer_domain(parsed.netloc) or _is_non_career_link_domain(parsed.netloc, policy):
        return 0
    url_haystack = f"{parsed.netloc} {parsed.path} {parsed.query}".casefold()
    url_match = any(marker in url_haystack for marker in policy.career_markers)
...
def _looks_like_career_url(url: str, policy: ApplicationChannelResolutionPolicy) -> bool:
    ...
    return any(domain in parsed.netloc.casefold() for domain in policy.ats_domains) or any(
        marker in haystack for marker in policy.career_markers
    )
...
def _is_non_career_link_domain(host: str, policy: ApplicationChannelResolutionPolicy) -> bool:
    folded = host.casefold()
