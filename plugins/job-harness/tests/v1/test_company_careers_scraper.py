...
from job_harness.v1.company_directory import CompanyProfile
from job_harness.v1.models import SearchParams
from job_harness.v1.registry import _SCRAPERS, register_scraper
from job_harness.v1.run_journal import RunJournalWriter
from job_harness.v1.scrapers.company_careers import CompanyCareersScraper
from job_harness.v1.search_engine import SearchEngine
from job_harness.v1.types import SearchCriterion, SearchRequest, SourceState
...
class CompanyCareersScraperTest(unittest.IsolatedAsyncioTestCase):
    async def test_converts_company_hits_to_listings_without_native_experience(self) -> None:
        ...
        with (
            patch("job_harness.v1.scrapers.company_careers._load_company_targets", return_value=[company]),
            patch("job_harness.v1.scrapers.company_careers._check_company", new_callable=AsyncMock) as check,
        ):
    ...
    async def test_scraping_scope_can_cover_all_selected_companies(self) -> None:
        ...
        with (
            patch("job_harness.v1.scrapers.company_careers._load_company_targets", return_value=companies),
            patch("job_harness.v1.scrapers.company_careers._check_company", new_callable=AsyncMock) as check,
        ):
