...
from job_harness.v1.models import RawListing
from job_harness.v1.run_journal import (
    EVENT_LISTING,
    EVENT_RUN_STARTED,
    EVENT_SOURCE_STATUS,
    JournalSnapshot,
...
class WriterBasicsTest(unittest.TestCase):
    ...
    def test_fsync_called_per_record(self):
        req = _make_request()
        with patch("job_harness.v1.run_journal.os.fsync") as fsync_spy:
            with RunJournalWriter(self.run_dir) as w:
                w.write_run_started(run_id="r-x", request=req)
...
from job_harness.v1.types import SearchRequest
...
class DiskFullSimulationTest(unittest.TestCase):
    def test_enospc_marks_writer_disk_full(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            with patch("job_harness.v1.run_journal.os.write", side_effect=OSError(28, "No space left on device")):
