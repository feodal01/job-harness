...
class SourceCatalogTableTest(unittest.TestCase):
    ...
    def test_coinspaid_catalog_row_declares_source_contract(self) -> None:
        # Arrange / Act
        descriptor = source_descriptor("career:coinspaid")
        required_fixture_kinds = source_required_fixture_kinds("career:coinspaid")
    ...
    def test_workday_catalog_rows_declare_native_query_contract(self) -> None:
        for source_id in ("career:semrush", "career:nvidia"):
            with self.subTest(source_id=source_id):
                ...
                self.assertIn(SearchCriterion.WORK_FORMATS, descriptor.structured_output_criteria)
