...
- `remote_in_country` is not a user intent. It is a source fact that becomes a
  limited remote scope such as `country:GB`, `country:PL`, or `region:europe`.
- `hybrid` and `office` are physical formats. They never become remote
  eligibility by themselves.
- The project-defined `europe` scope intentionally excludes `GB` and `RU`.
  Users who want the United Kingdom must request `UK` or `GB` explicitly.
...
8. Unknown geography or remote scope fails positive filters with an explicit
...
| `UK` | `europe` | remote | `remote_in_country=true`, scope `region:europe` | remove | The project `europe` scope excludes UK. |
| `UK` | `europe` | remote | scope `country:GB`, vacancy country includes `GB` and `europe` | keep | Remote scope covers UK and vacancy geography separately covers Europe. |
...
| `UK` | empty | remote | scope `country:PL` or `region:europe` | remove | Remote scope does not include UK. |
...
| empty | `europe` | no remote filter | country `GB` | remove | The project `europe` scope excludes UK. |
